...
export default function Dashboard(){
 ...
 const filtered=useMemo(()=>orders.filter(o=>{const q=search.trim().toLowerCase(),d=new Date(o.created_at),f=dateFrom?new Date(dateFrom+"T00:00:00"):null,t=dateTo?new Date(dateTo+"T23:59:59.999"):null;
  return(!q||String(o.order_number).includes(q)||(o.customer_name||"").toLowerCase().includes(q)||(o.customer_phone||"").toLowerCase().includes(q)||(o.tracking_code||"").toLowerCase().includes(q)||(o.profiles?.full_name||"").toLowerCase().includes(q))
  &&(status==="all"||o.status===status)&&(employee==="all"||o.profiles?.full_name===employee)&&(!f||d>=f)&&(!t||d<=t)
 }).sort((a,b)=>sort==="new"?+new Date(b.created_at)-+new Date(a.created_at):+new Date(a.created_at)-+new Date(b.created_at)),[orders,search,status,employee,dateFrom,dateTo,sort]);
 ...
 async function exportExcel(){
  ...
  try{const ids=filtered.map(o=>o.id),{data,error}=await createClient().from("order_items").select("order_id,product_name,quantity,unit_price,total_price").in("order_id",ids);if(error)throw error;const items=(data||[]) as Item[],rows:any[]=[];
   filtered.forEach(o=>{const oi=items.filter(i=>i.order_id===o.id),base={"შეკვეთის №":o.order_number,"მომხმარებელი":o.customer_name,"ტელეფონი":o.customer_phone,"თრექინგ კოდი":o.tracking_code||"","თანამშრომელი":o.profiles?.full_name||"","მიტანის საფასური (₾)":Number(o.delivery_fee||0),"შეკვეთის ჯამი (₾)":Number(o.total||0),"სტატუსი":labels[o.status],"თარიღი":new Date(o.created_at).toLocaleString("ka-GE")};
    if(oi.length<=5){const r:any={...base};for(let n=1;n<=5;n++)r[`პროდუქტი ${n}`]="";oi.forEach((i,n)=>r[`პროდუქტი ${n+1}`]=`${i.product_name} (x${i.quantity}, ${Number(i.unit_price).toFixed(2)} ₾)`);rows.push(r)}
    else oi.forEach((i,n)=>rows.push({...base,"პროდუქტის №":n+1,"პროდუქტი":i.product_name,"რაოდენობა":i.quantity,"ერთეულის ფასი (₾)":Number(i.unit_price),"პროდუქტის ჯამი (₾)":Number(i.total_price||0)}))
   });
 ...
 <div className="quick-filters">
  ...
  <button className="excel-btn" onClick={exportExcel} disabled={!filtered.length||exporting}>{exporting?"მზადდება...":"↓ Excel"}</button><button className="light-btn" onClick={clearFilters}>გასუფთავება</button>
 </div>
 {loading?<div className="simple-empty">იტვირთება...</div>:!visible.length?<div className="simple-empty">შეკვეთები ვერ მოიძებნა.</div>:<div className="simple-table-wrap"><table className="simple-table fast-table"><thead><tr><th>№</th><th>მომხმარებელი</th><th>ტელეფონი</th><th>თრექინგი</th><th>თანამშრომელი</th><th>ჯამი</th><th>სტატუსი</th><th>თარიღი</th><th></th></tr></thead><tbody>{visible.map(o=><tr key={o.id}><td><b>#{o.order_number}</b></td><td>{o.customer_name}</td><td><button className="copy-cell" title="კოპირება" onClick={()=>copy(o.customer_phone)}>{o.customer_phone}</button></td><td><input className="tracking-inline" defaultValue={o.tracking_code||""} placeholder="თრექინგი" onBlur={e=>{const v=e.target.value.trim();if(v!==(o.tracking_code||""))quickSave(o,{tracking_code:v||null})}}/></td><td>{o.profiles?.full_name||"—"}</td><td><b>{Number(o.total).toFixed(2)} ₾</b></td><td><select className={`status-inline ${o.status}`} value={o.status} disabled={saving===o.id} onChange={e=>quickSave(o,{status:e.target.value as Status})}>{statuses.map(s=><option key={s} value={s}>{labels[s]}</option>)}</select></td><td>{new Date(o.created_at).toLocaleDateString("ka-GE")}</td><td><Link className="view-btn" href={`/orders/${o.id}`}>ნახვა</Link></td></tr>)}</tbody></table></div>}
