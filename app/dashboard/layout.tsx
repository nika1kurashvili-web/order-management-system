...
export default function DashboardLayout({children}:{children:React.ReactNode}){
 ...
 return <div className="shell"><aside className="side"><div className="brand">📦 Orders</div><nav className="nav"><Link href="/dashboard">📊 Dashboard</Link><Link href="/orders/new">➕ ახალი შეკვეთა</Link><Link href="/products">🛒 პროდუქტები</Link>{role==="admin"&&<Link href="/employees">👥 თანამშრომლები</Link>}{role==="admin"&&<Link href="/reports">📈 ანალიტიკა</Link>}</nav><div className="role-chip">{role==="admin"?"Admin":"Operator"}</div><button className="btn secondary" style={{marginTop:12,width:"100%"}} onClick={logout}>გასვლა</button></aside><main className="main">{children}</main></div>
}
